...
parser.add_argument('--data', default='imdb', help='name of dataset')
...
parser.add_argument("--flows", default=3, type=int)
...
def main():
  ...
  for it in trange(n_iter):
    ...
    if it<thresh_iters:
          mu, logvar, z,recon_loss, kl_loss_z_prior = model.forward(inputs, labels_new, bert_inputs=sentence_embeddings, layer=1)
          lower_level_loss = recon_loss_weight * recon_loss + kld_weight * kl_loss_z_prior
    else:
          z, z_star, z_star_prior_logprob, z_star_log_det, recon_loss_f_c, recon_loss_f_s, z_decoded, z_log_det,kl_z_star = model.forward(inputs, labels_new, bert_inputs=sentence_embeddings, layer=2)

...
parser.add_argument('--data', default='amazon', help='name of dataset')
...
print("Dataset label:", dataset.LABEL.vocab.itos)
...
if with_bert:
  bert_sentence_embedding = torch.load("../BERT/amazon_debug_sentence.pt")
...
thresh_iters = n_batches*10 ## 10 epochs of lower level training
...
def main():
  ...
  kld_inc_f = (kld_max_f - kld_weight_f) / (kld_end_inc_f - kld_start_inc_f)
  ...
  loss_inc_f = (loss_max_f - loss_weight_f) / (loss_end_inc_f - loss_start_inc_f)
  ...
  for it in trange(n_iter):
    ...
    lower_level_loss = recon_loss + kld_weight * kl_loss_z_prior
    ...
    if (it % log_interval) == 0 or it == (n_iter-1):
      ...
      print('Iter-{}; Sentence Loss: {:.4f}; Recon: {:.4f}; KL_z_prior: {:.10f}; KLD_weight: {:.10f}'.format(
          it, loss_z.data, recon_loss.data, kl_loss_z_prior.data, kld_weight))
      print('Iter-{}; Feature Loss: Recon: {:.4f}; Weight:{:.4f}'.format(it, loss_f.data, loss_f_s_weight))

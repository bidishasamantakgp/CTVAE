...
#from model_flow_imdb_2 import RNN_VAE
...
parser.add_argument('--data', default='hs', help='name of dataset')
...
if args.data.lower() == 'hs':
  #dataset = Amazon_Dataset(mbsize=mb_size)
  #MODEL_DIR = 'models_amazon/'
  print('using data hs')
  #dataset = HS_Dataset(mbsize=mb_size, repeat=False, shuffle=False)
  dataset = HS_Dataset(mbsize=mb_size)
  MODEL_DIR = 'models_hs/'
  train_size = 23817
  val_size = 2000
...
if with_bert:
  bert_sentence_embedding = torch.load("../BERT/hs_train_sentence.pt")
...
thresh_iters = n_batches*50 ## 10 epochs of lower level training
...
#model.load_state_dict(torch.load("ny_flow_hs5/vae29799.bin"))
#model.load_state_dict(torch.load(args.save_dir+"/vae221550.bin"))
...
def main():
  ...
  #kld_end_inc = 0.6*thresh_iters
  #kld_weight = 0.01

  kld_start_inc = 0.25 * thresh_iters
  ...
  kld_start_inc_f =  thresh_iters + (n_iter - thresh_iters)* 0.5 
  kld_end_inc_f = kld_start_inc_f + 0.8*(n_iter-kld_start_inc_f)
  kld_weight_f = 0.001
  kld_max_f = 0.1
  ...
  loss_start_inc_f = thresh_iters + (n_iter - thresh_iters) * 0.5 
  loss_end_inc_f = loss_start_inc_f + 0.8*(n_iter-loss_start_inc_f )
  loss_weight_f = 0.01
  loss_max_f = 1

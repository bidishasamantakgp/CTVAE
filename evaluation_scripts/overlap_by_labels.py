...
# for the sent 2d array
# fill in zero (classification) scores where
# (output) sentence is blank
if len(sent) != len(output):
  _sent = np.copy(sent)
  sent = np.zeros((len(output), 2))

  j = 0
  for i, d in enumerate(output):
    if not d:
      # print('no output data for i:',i)
      continue
    sent[i, :] = _sent[j, :]
    j += 1
...
sent = sent.reshape((-1, num_per_sample, 2))
# sent_neg_mask = sent<0.5
# sent_pos_mask = sent>0.5
sent_neg_mask = sent[:, :, 0] >= 0.5
sent_pos_mask = sent[:, :, 1] > 0.5
...
# print('{:.3f}, {:.4f}'.format(np.mean(scores_neg_sent), np.std(scores_neg_sent)))
# print('{:.3f}, {:.4f}'.format(np.mean(scores_pos_sent), np.std(scores_pos_sent)))
# print()
